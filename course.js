...
window.CourseTracker = (function () {
  ...
  // {section, text, options: [...], correctIndex, explain}) в отдельном
  // файле, например question-bank-module6.js. На странице модуля вызывается:
  ...
  // .question-элементов, с которыми дальше работает обычная логика
  // проверки (checkBtn/retryBtn) — её менять не нужно.
  ...
  function renderQuiz(bank, containerEl, count, namePrefix) {
    ...
    picked.forEach(function (q, idx) {
      // перемешиваем порядок вариантов ответа, но не теряем, какой из них верный
      var optOrder = shuffle(q.options.map(function (_, i) { return i; }));
      var newCorrectIndex = optOrder.indexOf(q.correctIndex);

      var qDiv = document.createElement('div');
      qDiv.className = 'question';
      qDiv.setAttribute('data-correct', newCorrectIndex);
      ...
      html += '<p class="question-title">' + (idx + 1) + '. ' + q.text + '</p>';
      optOrder.forEach(function (origIndex, i) {
        html += '<label class="option"><input type="radio" name="' + qname + '" value="' + i + '"><span>' +
